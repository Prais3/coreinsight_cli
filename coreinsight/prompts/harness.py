"""
coreinsight/prompts/harness.py
Harness generation prompts — Agent 2 in multi-agent pipeline.
Also contains single-agent HARNESS_ADDENDUM variants.
"""
...
INSTRUCTIONS:
1. Actively hunt for Medium, High, and Critical issues based ONLY on the specific {language} rubric above.
2. If you find an issue, you MUST explain the hardware-level or interpreter-level reasoning clearly.
3. SEVERITY BIAS: When uncertain between two severity levels, always choose the higher one. Only assign Low severity if you can explicitly prove the algorithm is already optimal.
4. CODE GENERATION MANDATE: You MUST provide the completely rewritten, optimized function in the `optimized_code` field. Raw, syntactically correct {language} code only. No markdown backticks inside the JSON string.
"""

# ── Single-agent harness templates ───────────────────────────────────────────
HARNESS_TEMPLATE = """
...
Write the complete executable script that:
...
5. Target Hardware: {hardware_target}. The largest N MUST cross cache boundaries but MUST NOT exceed 20% of available RAM.
...
# ── Multi-agent harness fix template ────────────────────────────────────────
FIX_TEMPLATE_MULTI = """
...
3. No internet. Stdlib only.
4. Print CSV header first. If any N crashes, print a row of zeros and continue.
""",
    ModelTier.MEDIUM: """
REMINDERS:
- Paste both functions inline before calling them
